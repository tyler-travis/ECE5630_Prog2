...
\newcommand{\hmwkTitle}{Programming\ \#2} % Assignment title
\newcommand{\hmwkDueDate}{Tuesday,\ November\ 24,\ 2014} % Due date
...
\begin{homeworkProblem}
  ...
  \begin{itemize}
    \item 256 coefficients
    \item Lowpass filter
    \item Passband frequency of 300 Hz with unit gain.
    \item Stopband frequency of 400 Hz with zero gain.
    \item A signal sample rate of 11.025 kHz.
  \end{itemize}
...
\begin{homeworkProblem}
  ...
  \begin{center}
    \noindent\rule{6.5in}{0.4pt}
  \end{center}
  Listing \ref{src/part1} shows the first program.

  \cppscript{src/part1}{Program 1 - part1.cpp}
  ...
  \begin{homeworkSection}{(c)}
    ...
    \begin{center}
      ...
      \hline
      & 10Hz & 40Hz & 150Hz & 350Hz & 500Hz \\ \hline
      Matlab & 1.0148 & 0.9925 & 0.9875 & 0.3674 & 0.0016 \\ \hline
      Generated & 1.0156 & 0.9916 & 0.9874 & 0.3681 & 0.0022 \\ \hline
      \end{tabular}
    \end{center}
...
\begin{homeworkProblem}
  ...
  \begin{itemize}
    \item You may do \textbf{either} overlap-add or overlap-save in your program.
    \item You only need to perform the FFT on the lowpass filter once.
    \item The FFT must be a power of two.
    \item The number of operations/output sample will vary as a function of the length of the FFT used.
    \item Your program should be written so that you can filter an infinite length signal.
  \end{itemize}
  \begin{center}
    \noindent\rule{6.5in}{0.4pt}
  \end{center}
...
\begin{homeworkProblem}
  Use the Matlab function wavread() to generate the samples of the file galway11\_mono\_45sec.wav. Use your programs from 2 and 3 above to filter the sound file. The results should be the same for both programs. Does the filter remove the high frequency components? Does the processed file sound as you expected? Write out the final results in a .wav file for the instructor to listen to.

  \begin{center}
    \noindent\rule{6.5in}{0.4pt}
  \end{center}
  The filter removes the high frequency components from the signal. You can no longer
  hear the flute playing in the foreground. You can only hear the bass notes of deeper
  instruments. The processed file sounds like what I expected, the filter completely
  took out the higher sounds.
  \begin{figure}[ht]
    \centering
    \begin{subfigure}[b]{0.4\textwidth}
      \includegraphics[width=\textwidth]{plots/Part4-input}
      \caption{x[n]}
      \label{[fig:4input]}
    \end{subfigure}
    \begin{subfigure}[b]{0.4\textwidth}
      \includegraphics[width=\textwidth]{plots/Part4-output-time_conv}
      \caption{y[n] time convolution}
      \label{[fig:4outputTime]}
    \end{subfigure}
    \begin{subfigure}[c]{0.4\textwidth}
      \includegraphics[width=\textwidth]{plots/Part4-output-freq_conv}
      \caption{y[n] frequency convolution}
      \label{[fig:4outputFreq]}
    \end{subfigure}
    \caption{Input(a) and Output of Time Convolution (b) and Output of Frequency Convolution (c)}
  \end{figure}
\end{homeworkProblem}
\end{document}
